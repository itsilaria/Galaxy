...
// Funzione per inizializzare il database con segreti mock
async function initializeMockSecrets() {
  const mockSecrets = generateMockSecrets(100);
  
  // Salva tutti i segreti mock nel database
...
// Genera segreti mock deterministici
function generateMockSecrets(count: number) {
  ...
  const contentPool = {
    ...
    it: ["Non ho detto ai miei che ho perso il lavoro.", "Sono innamorato della mia migliore amica.", "Voglio trasferirmi su Marte.", "Ho rubato un palloncino a 5 anni.", "A volte vorrei solo sparire.", "Ho paura del buio."],
  ...
  for (let i = 0; i < count; i++) {
    ...
    secrets.push({
      ...
      timestamp: Date.now() - (count - i) * 100000,
      language: lang,
    });
  }
