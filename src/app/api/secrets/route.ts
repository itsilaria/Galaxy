...
export async function POST(request: NextRequest) {
  try {
    ...
    const { text, position, color, language, isPremium, starType, glowColor } = body;
    ...
    // Salva il segreto nel database Redis (HashSet per scalabilità)
...
export async function GET() {
  try {
    ...
    if (!secretsObj || Object.keys(secretsObj).length === 0) {
      // Se il database è proprio vuoto, carichiamo quelli iniziali
      secrets = await initializeMockSecrets();
    } else {
      secrets = Object.values(secretsObj).map((s) => JSON.parse(s as string));
    }
    
    // Ordiniamo per data (i più recenti rimangono)
...
function generateMockSecrets(count: number) {
  ...
  const contentPool = {
    en: ["I haven't told my parents I lost my job.", "I'm in love with my best friend.", "I want to move to Mars."],
    it: ["Non ho detto ai miei che ho perso il lavoro.", "Sono innamorato della mia migliore amica.", "Voglio trasferirmi su Marte.", "Ho paura del buio."],
    es: ["No le he dicho a mis padres que perdí mi trabajo.", "Estoy enamorado de mi mejor amigo."],
    fr: ["Je n'ai pas dit à mes parents que j'ai perdu mon emploi."],
    de: ["Ich habe meinen Eltern nicht gesagt, dass ich meinen Job verloren habe."],
    jp: ["両親に仕事を失ったことを言っていません。"]
  };
